...
abstract class BaseModel {
    ...
    final public static function retrieveDbName(): string
    {
        ...
        $dbName = '\Classes\Main\Db' . ucfirst(strtoLower($tmpClass[$length - 2]));
        return $dbName;
    }
     ...
     *                                  false: wenn nicht
     */
    protected function validateInt(?int $val, bool $nullable = false, ?int $maxValue = null, $lessThanZero = false) : bool {
        if($val === null && $nullable === true) {
            return true;
        }
        else {
     ...
     *                                   false: wenn nicht
     */
    protected function validateDateTime(?DateTime $val, bool $nullable = false, ?DateTime $minDate = null, ?DateTime $maxDate = null) : bool {
        if($val === null && $nullable === true) {
            return true;
        }
        else {
     ...
     *                                    false: wenn nicht
     */
    protected function validateFloat(?float $val, bool $nullable = false, int $preDigits = 4, int $postDigits = 2, $lessThanZero = false) : bool {
        if($val === null && $nullable === true) {
            return true;
        }
        else {
